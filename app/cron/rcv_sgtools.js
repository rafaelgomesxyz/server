...
async function updateRcvSgTools(connection) {
	...
	for (const game of games) {
		...
		const effectiveDate = Math.trunc(
			new Date(`${game.bundled_date.replace(/\//g, '-').replace(/\s/, 'T')}.000Z`).getTime() / 1e3
		);
		const addedDate = Math.trunc(
			new Date(`${game.creation_date.replace(/\//g, '-').replace(/\s/, 'T')}.000Z`).getTime() / 1e3
		);
						...
						.map(
							(rcv) =>
								`(${connection.escape(rcv.id)}, ${connection.escape(
									rcv.effectiveDate
								)}, ${connection.escape(rcv.addedDate)}, TRUE)`
						)
						.join(', ')}
